...
userApp.get("/sign-in-admin/:username-:password", async (req, res) => {
  //check null data
  let username = req.params.username;
  if (!username)
  {
    // admin.auth().createCustomToken(req.url).then((customToken) => {
    //   console.log(customToken);
    // });
    // admin.auth().verifyIdToken("123", a).then()
...
//because get method in unity can not send body
userApp.get("/sign-in/:username-:password", async (req, res) => {
  //check null data
  let username = req.params.username;
  if (!username)
  {
